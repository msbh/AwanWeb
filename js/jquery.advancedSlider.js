...
(function(h) {
	...
	function AdvancedSlider(instance, options) {
		...
		var VERSION = '3.0',
			...
			slideProps = ['alignType', 'effectType', 'sliceEffectType', 'sliceDelay', 'sliceDuration', 'sliceEasing', 'horizontalSlices', 'verticalSlices', 'slicePattern', 'slicePoint', 'sliceStartPosition',
						  'sliceStartRatio', 'sliceFade', 'captionSize', 'captionPosition', 'captionShowEffectDuration', 'captionShowEffectEasing', 'captionHideEffectDuration', 'captionHideEffectEasing', 
						  ...
						  'captionBackgroundColor', 'captionBackgroundOpacity', 'slideshowDelay', 'slideMask', 'slideDirection', 'slideDuration', 'slideEasing', 'fadeInDuration', 
							...
							$(xml).find('slide').each(function(index) {
								...
								if (isImage && xmlLink.length)
									link = $('<a></a>').attr('href', xmlLink.text())
													   ...
													   .attr('rel', (xmlLink.attr('type') == 'slider-lightbox' ? xmlLink.attr('type') : '') + ((xmlLink.attr('group') ? '[' + xmlLink.attr('group') + ']' : '')))
													   .appendTo(slide);
		...
		function create() {
			...
			slideWrapper = $('<div class="slide-wrapper"></div>')
				...
				.hover(function() {
					...
					if (self.settings.pauseSlideshowOnHover)
						pauseSlideshow();
					...
					function() {
						...
						if (self.settings.pauseSlideshowOnHover && !isLightbox)
							resumeSlideshow();
			...
			slides.find('.slide').each(function(index) {
				...
				if (caption.length) {
					var	captionBackgroundOpacity = properties.captionBackgroundOpacity,
			...
			if (self.settings.lightbox)
				slider.find("a[rel^='slider-lightbox']").prettyPhoto({social_tools: '', 
																      ...
																      default_width: self.settings.lightboxDefaultWidth,
																      default_width: self.settings.lightboxDefaultHeight,
		...
		*/
		function doSliderLayout() {			
			...
			if (self.settings.slideButtons && (self.settings.slideButtonsContainerCenter || self.settings.slideButtonsCenter)) {
			...
			slides.find('.slide').each(function(index) {
				...
				if (image.length && (image[0].complete || image[0].readyState == 'complete'))
					scaleAndAlign(image, alignType);
					
				if (self.settings.effectType == 'swipe') {
					var slideIndex = currentIndex != -1 ? currentIndex : self.settings.slideStart;
			...
			if (self.settings.thumbnailType == 'tooltipAndScroller' || self.settings.thumbnailType == 'scroller') {
				...
				case 'proportionalFit':
					if (image.data('index') == currentIndex && (image[0].complete || image[0].readyState == 'complete')) {
						...
						if (image.width() / image.height() >= slideWrapper.width() / slideWrapper.height()) {
							...
							image.css({width: 'auto', height: '100%'});	
							slideWrapper.css('width', image.width());
						}
						
						slideWrapper.css('margin-left', (slideWrapperWidth - slideWrapper.width()) * 0.5);
						slideWrapper.css('margin-top', (slideWrapperHeight - slideWrapper.height()) * 0.5);
		...
		*/
		function showThumbnail(index) {
			...
			if (isOldIE) {						
				...
				thumbnailCaptionBackground.animate({'opacity': opacity}, self.settings.thumbnailSlideDuration);
			} else {
				thumbnailWrapper.animate({'top': topPos, 'opacity': 1}, 
										  self.settings.thumbnailSlideDuration, 
										  self.settings.thumbnailSlideEasing);
		...
		*/
		function gotoSlide(index) {
			...
			if (slideData.lazyImage && !slideData.lazyImageLoaded) {
				...
				if (image.length) {
					newImage.appendTo(image.parent());
					image.remove();
				} else {
					newImage.appendTo(slideDivs[index]);
				}
			...
			if (self.settings.thumbnailType == 'scroller' || self.settings.thumbnailType == 'tooltipAndScroller') {				
				...
				if (self.settings.thumbnailSync) {
					var thumbnailPage = Math.floor(index / self.settings.maximumVisibleThumbnails);
			...
			for (var i = 0; i < imagesToPreload.length; i++) {
				...
				if (slideData.lazyImage && !slideData.lazyImageLoaded) {
					...
					if (image.length) {
						...
						newImage.appendTo(slideDivs[idx]);
					}
		...
		*/
		function showPreloader() {
			var preloader = $('<div class="preloader"></div>')
				...
				preloaderLeft = ((slideWrapper.outerWidth(true) -preloader.width()) * 0.5),
				preloaderTop = ((slideWrapper.outerHeight(true) - preloader.height()) * 0.5);
		...
		function startSlideTransition() {
			...
			switch (slideDirection) {
				case 'autoHorizontal':
					...
					if (currentIndex == 0 && previousIndex == numSlides - 1)
						value = slideWrapper.width();
					else if (currentIndex == numSlides - 1 && previousIndex == 0)
						value = - slideWrapper.width();
					else
		...
		*/
		function startSwipeTransition() {				
			if (previousIndex == -1) {
				var container = $('<div class="swipeable-container"></div>')
				...
				if (self.settings.swipeOrientation == 'horizontal') {
					slides.css('left', - self.settings.slideStart * (slideWrapper.width() + self.settings.swipeSlideDistance));
					...
					slides.css('top', - self.settings.slideStart * (slideWrapper.height() + self.settings.swipeSlideDistance));
		...
		*/
		function startSliceTransition() {
			...
			slideClone.css('visibility', 'visible').find('.caption').remove();
		...
		*/
		function completeTransition() {
			var slideData = slidesData[currentIndex],
				...
				captionDelay = slideData.properties.captionDelay;
			...
			if ($.fn.videoController && slideDivs[currentIndex].find('.video').length) {
				currentVideo = slideDivs[currentIndex].find('.video').videoController({
					...
					start: function() {
						if (self.settings.videoPlayAction == 'stopSlideshow')
							clearSlideshow();
						else if (self.settings.videoPlayAction == 'pauseSlideshow')
							pauseSlideshow();
					},
					play: function() {
						if (self.settings.videoPlayAction == 'stopSlideshow')
					...
					pause: function() {
						if (self.settings.videoPauseAction == 'startSlideshow')
							resetSlideshow();
						else if (self.settings.videoPauseAction == 'resumeSlideshow')
							resumeSlideshow();
					},
					end: function() {
						if (self.settings.videoEndAction == 'resetVideo')
							currentVideo.stopVideo();
						else if (self.settings.videoEndAction == 'startSlideshow')
							resetSlideshow();
						else if (self.settings.videoEndAction == 'resumeSlideshow')
							resumeSlideshow();
		...
		*/
		function hideCaption() {
			...
			if (captionHideEffect == "fade") {
				if (isOldIE) {
					if ($.browser.version != '6.0') {
						captionContent.animate({'opacity': 0}, captionHideEffectDuration, captionHideEffectEasing);						
					...
					captionWrapper.animate({'opacity': 0}, captionHideEffectDuration, captionHideEffectEasing);
				}
		...
		*/
		function pauseSlideshow() {
			if (self.settings.slideshow && slideshowState != 'stopped' && !isTransition) {
				if (slideshowTimer)
					clearTimeout(slideshowTimer);
		...
		*/
		function startTimerAnimation(delay) {
			...
			var	ctx = timerCanvas.getContext ? timerCanvas.getContext("2d") : G_vmlCanvasManager.initElement(timerCanvas).getContext("2d");	
			...
			timerAnimationTimer = setInterval(function() {
				if (angle <= 360 && !((self.settings.pauseSlideshowOnHover && isHover) || isLightbox || slideshowState == 'paused')) {
		...
		*/
		function createThumbnailScroller() {
			...
			for (var i = 0; i < numSlides; i++) {
				if (slidesData[i].thumbnail && (slidesData[i].properties.thumbnailType == 'scroller' || slidesData[i].properties.thumbnailType == 'tooltipAndScroller')) {
					...
					if (self.settings.thumbnailOrientation == 'horizontal')
						thumbnailWrapper.css('left', numThumbnails * thumbnailWrapper.outerWidth(true));
					else
						thumbnailWrapper.css('top', numThumbnails * thumbnailWrapper.outerHeight(true));
					...
					thumbnail.hover(
						function() {									
							...
							var thumbnailCaption = $(this).find('.caption'),
								thumbnailCaptionBackground = thumbnailCaption.find('.background'),
								thumbnailCaptionContent = thumbnailCaption.find('.content'),
							...
							if (thumbnailCaption)
								if (self.settings.thumbnailCaptionToggle)
									if (self.settings.thumbnailCaptionEffect == 'slide') {
										...
										if (isOldIE) {
											thumbnailCaptionBackground.stop().animate({'opacity': oldIEOpacity}, self.settings.thumbnailCaptionShowDuration, self.settings.thumbnailCaptionEasing);
											thumbnailCaptionContent.stop().animate({'opacity': 1}, self.settings.thumbnailCaptionShowDuration, self.settings.thumbnailCaptionEasing);
										} else {
							...
							if (thumbnailCaption)
								if (self.settings.thumbnailCaptionToggle)
									if (self.settings.thumbnailCaptionEffect == 'slide') {
										...
										if (isOldIE) {
											thumbnailCaptionBackground.stop().animate({'opacity': 0}, self.settings.thumbnailCaptionHideDuration, self.settings.thumbnailCaptionEasing);
											thumbnailCaptionContent.stop().animate({'opacity': 0}, self.settings.thumbnailCaptionHideDuration, self.settings.thumbnailCaptionEasing);
										} else {
			...
			totalThumbnailPages = Math.ceil(numThumbnails / self.settings.maximumVisibleThumbnails);
			
			
			// set the initial width and height of the thumbnail scroller and thumbnail container
			if (self.settings.thumbnailOrientation == 'horizontal') {				
				thumbnailScroller.css({'height': thumbnailHeight, 'width': thumbnailWidth * self.settings.maximumVisibleThumbnails});																			 
				thumbnailsVisibleContainer.css({'height': thumbnailHeight, 'width': thumbnailWidth * self.settings.maximumVisibleThumbnails});				
			} else {
				thumbnailScroller.css({'width': thumbnailWidth, 'height': thumbnailHeight * self.settings.maximumVisibleThumbnails});				
				thumbnailsVisibleContainer.css({'width': thumbnailWidth, 'height': thumbnailHeight * self.settings.maximumVisibleThumbnails});				
			}
		...
		function doThumbnailScrollerLayout() {			
			...
			if (self.settings.thumbnailOrientation == 'horizontal') {
				...
				thumbnailsTotalSize = numThumbnails * thumbnailWidth;
				visibleThumbnails = self.settings.thumbnailScrollerResponsive ? 
									Math.max(self.settings.minimumVisibleThumbnails, 
											 Math.min(numThumbnails, self.settings.maximumVisibleThumbnails, Math.floor((slideWrapper.outerWidth() - arrowsSize) / thumbnailWidth))) : 
				...
				thumbnailsTotalSize = numThumbnails * thumbnailHeight;
				visibleThumbnails = self.settings.thumbnailScrollerResponsive ?
									Math.max(self.settings.minimumVisibleThumbnails,  
											 Math.min(numThumbnails, self.settings.maximumVisibleThumbnails, Math.floor((slideWrapper.outerHeight() - arrowsSize) / thumbnailHeight))) : 
									Math.min(numThumbnails, self.settings.maximumVisibleThumbnails); 
			...
			if (previousVisibleThumbnails != visibleThumbnails) {
				...
				totalThumbnailPages = Math.ceil(numThumbnails / visibleThumbnails);
		...
		function createThumbnailArrows() {
			...
			if (self.settings.thumbnailOrientation == 'horizontal') {						
				previousArrow.css({'top': Math.round((thumbnailHeight - previousArrow.height()) / 2)});
				nextArrow.css({'top': Math.round((thumbnailHeight - nextArrow.height()) / 2)});
			} else {
				previousArrow.css({'left': Math.round((thumbnailWidth - previousArrow.width()) / 2)});
				nextArrow.css({'left': Math.round((thumbnailWidth - nextArrow.width()) / 2)});
			}
		...
		function createThumbnailButtons() {
			...
			if (thumbnailScrollerButtons.find('.buttons a').length != totalThumbnailPages) {
				...
				if (self.settings.thumbnailOrientation == 'horizontal') {	
					thumbnailScrollerButtons.css('top', thumbnailHeight);					
					...
					thumbnailScrollerButtons.css('left', thumbnailWidth);
		...
		*/
		function createScrollbar() {
			...
			if (self.settings.thumbnailOrientation == 'horizontal') {
				...
				thumbnailScrollerScrollbar.css({'top': thumbnailHeight, 'left': (thumbnailScroller.outerWidth() - thumbnailScrollerScrollbar.width()) / 2});
				...
				thumbnailScrollerScrollbar.css({'left': thumbnailWidth, 'top': (thumbnailScroller.outerHeight() - thumbnailScrollerScrollbar.height()) / 2});
		...
		*/
		function showTooltip(index) {
			var tooltipContent = slidesData[index].thumbnailTooltip;
	...
	$.fn.advancedSlider.defaults =  {
		...
		scaleType: 'none', // none, exactFit, insideFit, outsideFit, proportionalFit
		...
		leaveVideoAction: 'stopVideo', // stopVideo, pauseVideoAndBuffering, pauseVideo
